...
import { CalendarDays, Edit3, FileText, Megaphone, BellRing, Zap, MessageCircle, UserCircle, Car } from 'lucide-react';
...
const DashboardPage = () => {
  const featureCards = [
    ...
    },
    {
      ...
      icon: Megaphone,
      actionIcon: Megaphone, // 혹은 다른 아이콘
  ...
  const notifications = [
    { title: '헬스장 예약이 승인되었습니다.', details: '2024-03-20 14:00-16:00', time: '1시간 전' },
    { title: '3월 관리비 고지서가 발행되었습니다.', details: '납부 마감일: 2024-03-25', time: '3시간 전' },
    { title: '커뮤니티센터 대청소 안내', details: '내일 오전 10시부터', time: '5시간 전' },
  ];
    ...
    <div className="flex bg-gray-50 min-h-screen">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {featureCards.slice(0, 2).map((card) => (
           ...
           <FeatureCard
              ...
              iconBgColor={featureCards[3].iconBgColor}
              iconColor={featureCards[3].iconColor}
              actionIcon={featureCards[3].actionIcon}
            />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          {featureCards.slice(2, 3).map((card) => ( // 점검관리
            <FeatureCard
              key={card.title}
              title={card.title}
              description={card.description}
          ...
          {featureCards.slice(4, 5).map((card) => ( // 공지사항
             <FeatureCard
              key={card.title}
              title={card.title}
              description={card.description}
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg">
          <h3 className="text-xl font-semibold text-gray-800 mb-4">최근 알림</h3>
          <div>

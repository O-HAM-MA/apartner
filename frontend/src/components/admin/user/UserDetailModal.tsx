...
import { Modal, Tabs, Descriptions, Badge, Spin, message, Select, Tag, Button } from 'antd';
...
const UserDetailModal: React.FC<UserDetailModalProps> = ({ userId, open, onClose, onUserUpdated }) => {
  ...
  const handleStatusUpdate = async () => {
    if (!userId || !userDetail || !selectedStatus || selectedStatus === userDetail.status) return;
    
    Modal.confirm({
      title: '사용자 상태 변경',
      content: `사용자 상태를 ${getStatusText(userDetail.status)}에서 ${getStatusText(selectedStatus)}로 변경하시겠습니까?`,
      okText: '변경',
      cancelText: '취소',
      onOk: async () => {
        try {
        ...
        <>
          <Descriptions bordered column={2} size="small" className="mb-6">
            ...
            <Descriptions.Item label="가입 방식">
              {userDetail.socialProvider ? `소셜 로그인 (${userDetail.socialProvider})` : '일반 가입'}
            </Descriptions.Item>
                  ...
                  >
                    <Option value={UserStatus.ACTIVE}>
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        <span style={{ marginLeft: 5 }}>{getStatusText(UserStatus.ACTIVE)}</span>
                      </div>
                    </Option>
                    <Option value={UserStatus.INACTIVE}>
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        <MinusCircleFilled style={{ color: 'gray' }} />
                        <span style={{ marginLeft: 5 }}>{getStatusText(UserStatus.INACTIVE)}</span>
                      </div>
                    </Option>
                    <Option value={UserStatus.SUSPENDED}>
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        <CloseCircleFilled style={{ color: 'red' }} />
                        <span style={{ marginLeft: 5 }}>{getStatusText(UserStatus.SUSPENDED)}</span>
                      </div>
                    </Option>
                    {userDetail.status === UserStatus.DELETED && (
                      <Option value={UserStatus.DELETED}>
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                          ...
                          <span style={{ marginLeft: 5 }}>{getStatusText(UserStatus.DELETED)}</span>
                        </div>
                      </Option>
                    )}
                  </Select>
            ...
            <Descriptions.Item label="가입 일시">
              {userDetail.createdAt ? dayjs(userDetail.createdAt).format('YYYY-MM-DD HH:mm:ss') : '-'}
            </Descriptions.Item>
            <Descriptions.Item label="최근 로그인">
              {userDetail.lastLoginAt ? dayjs(userDetail.lastLoginAt).format('YYYY-MM-DD HH:mm:ss') : '-'}
            </Descriptions.Item>
            {userDetail.status === UserStatus.DELETED && (
              <Descriptions.Item label="탈퇴 일시" span={2}>
                {userDetail.deletedAt ? dayjs(userDetail.deletedAt).format('YYYY-MM-DD HH:mm:ss') : '-'}
              </Descriptions.Item>
            )}
          </Descriptions>
        </>
      ...
      <Spin spinning={loading}>
        {userDetail ? (
          ...
          <div className="py-8 text-center text-gray-500">
            {loading ? '사용자 정보를 불러오는 중...' : '사용자 정보가 없습니다'}
          </div>
        )}
      </Spin>
    </Modal>
